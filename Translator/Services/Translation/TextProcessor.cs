...
public class TextProcessor : ITextProcessor
{
    ...
    public (string word, string punctuation) SplitWordFromPunctuation(string input)
    {
        ...
        while (lastLetterIndex >= 0 && !char.IsLetterOrDigit(input[lastLetterIndex])) lastLetterIndex--;

        if (lastLetterIndex < 0) return (string.Empty, input);

        var word = input.Substring(0, lastLetterIndex + 1);
        var punctuation = input.Substring(lastLetterIndex + 1);
